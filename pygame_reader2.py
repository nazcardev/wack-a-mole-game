...
grid_height = 3 * mole_image.get_height() + 2 * MOLE_SPACING_Y
...
for row in range(3):
    for col in range(3):
        # Position each mole relative to the calculated top-left of the grid
        x = start_x + col * (mole_image.get_width() + MOLE_SPACING_X)
        y = start_y + row * (mole_image.get_height() + MOLE_SPACING_Y)
